...
OUTPUT_DIR = 'joai/exp0'  # Directory to save models and outputs
...
class GasDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        # Create datasets
        self.train_dataset = GasDataset(self.train_data, self.image_dir, transform=train_transform, preload=self.preload)
        self.val_dataset = GasDataset(self.val_data, self.image_dir, transform=test_transform, preload=self.preload)
        if self.test_df is not None:
            self.test_dataset = GasDataset(self.test_df, self.image_dir, transform=test_transform, is_test=True, preload=self.preload)
...
# Define the Lightning Module for the CNN model
...
for fold in range(NUM_FOLDS):
    ...
    # Create model
    model = GasClassifierModule(num_classes=4, learning_rate=LEARNING_RATE)
