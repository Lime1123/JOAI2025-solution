...
OUTPUT_DIR = 'joai/exp3'  # Directory to save models and outputs
...
# Load data
print("Loading data...")
train_df = pd.read_csv(os.path.join(DATA_DIR, 'train_with_folds.csv'))
test_df = None
...
class GasDataset(Dataset):
    def __init__(self, dataframe, image_dir, transform=None, is_test=False, preload=True):
        self.df = dataframe
...
class GasDataModule(pl.LightningDataModule):
    def __init__(self, df, test_df, image_dir, fold_idx=None, batch_size=32, preload=True):
        super().__init__()
        self.df = df
    ...
    def setup(self, stage=None):
        ...
        # Create datasets
        self.train_dataset = GasDataset(self.train_data, self.image_dir, preload=self.preload)
        self.val_dataset = GasDataset(self.val_data, self.image_dir, preload=self.preload)
        if self.test_df is not None:
            self.test_dataset = GasDataset(self.test_df, self.image_dir, is_test=True, preload=self.preload)
...
# Define the Lightning Module for the CLIP ViT model
class CLIPVitClassifierModule(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        # Apply our classifier to the image features
...
for fold in range(NUM_FOLDS):
    ...
    data_module = GasDataModule(train_df, test_df, IMAGE_DIR, fold_idx=fold, batch_size=BATCH_SIZE, preload=PRELOAD_IMAGES)
    data_module.setup()
    ...
    # Create model
    model = CLIPVitClassifierModule(num_classes=4, learning_rate=LEARNING_RATE)
    
    # Define callbacks
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(OUTPUT_DIR, 'checkpoints', f'fold_{fold}'),
        filename='clip-vit-classifier-{epoch:02d}-{val_f1:.4f}',
        save_top_k=1,
        verbose=True,
        monitor='val_f1',
        mode='max'
    ...
    # Create logger
    logger = TensorBoardLogger(os.path.join(OUTPUT_DIR, 'lightning_logs'), name=f'clip_vit_classifier_fold_{fold}')
    ...
    best_model = CLIPVitClassifierModule.load_from_checkpoint(best_model_path)
    best_model.eval()
