...
OUTPUT_DIR = 'joai/exp13'  # Directory to save models and outputs
...
# Define the Lightning Module for the DeBERTa model
class DeBERTaClassifierModule(pl.LightningModule):
    def __init__(self, model_name='microsoft/deberta-base', num_classes=4, learning_rate=2e-5):
        super().__init__()
...
for fold in range(NUM_FOLDS):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='deberta-classifier-{epoch:02d}-{val_f1:.4f}',
        save_top_k=1,
        verbose=True,
        monitor='val_f1',
        mode='max'
    ...
    # Create logger
    logger = TensorBoardLogger(os.path.join(OUTPUT_DIR, 'lightning_logs'), name=f'deberta_classifier_fold_{fold}')
    ...
    best_model = DeBERTaClassifierModule.load_from_checkpoint(best_model_path)
    best_model.eval()
