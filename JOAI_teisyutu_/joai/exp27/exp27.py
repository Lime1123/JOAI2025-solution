...
# Define paths
DATA_DIR = '/workspace/JOAI/data'  # Update this path if your data is located elsewhere
OUTPUT_DIR = '/workspace'  # Directory to save models and outputs
...
mq8_mean = train_df['MQ8'].replace([np.inf, -np.inf], np.nan).mean(skipna=True)
mq5_mean = train_df['MQ5'].replace([np.inf, -np.inf], np.nan).mean(skipna=True)
...
train_df['MQ8'] = train_df['MQ8'].replace([np.inf, -np.inf], np.nan).fillna(mq8_mean)
train_df['MQ5'] = train_df['MQ5'].replace([np.inf, -np.inf], np.nan).fillna(mq5_mean)
...
if test_df is not None:
    ...
    test_df['MQ8'] = test_df['MQ8'].replace([np.inf, -np.inf], np.nan).fillna(mq8_mean)
    test_df['MQ5'] = test_df['MQ5'].replace([np.inf, -np.inf], np.nan).fillna(mq5_mean)
...
class GasDataset(Dataset):
    ...
    def _add_feature_channels(self, img, idx):
        """Add MQ8, MQ5, temp_min, temp_max, temp_unit, and has_coordinates as additional channels"""
        row = self.df.iloc[idx]
        
        # Get feature values
        temp_min = row.get('temp_min', self.feature_stats['temp_min_mean'])
        temp_max = row.get('temp_max', self.feature_stats['temp_max_mean'])
        mq8 = row.get('MQ8', self.feature_stats['mq8_mean'])
        mq5 = row.get('MQ5', self.feature_stats['mq5_mean'])
        ...
        if pd.isna(mq8) or np.isinf(mq8):
            mq8 = self.feature_stats['mq8_mean']
        if pd.isna(mq5) or np.isinf(mq5):
            mq5 = self.feature_stats['mq5_mean']
        ...
        # Standardize values using precomputed statistics
        ...
        mq8_standardized = (float(mq8) - self.feature_stats['mq8_mean']) / self.feature_stats['mq8_std']
        mq5_standardized = (float(mq5) - self.feature_stats['mq5_mean']) / self.feature_stats['mq5_std']
        ...
        mq8_channel = np.ones(img.shape[:2], dtype=np.float32) * mq8_standardized
        mq5_channel = np.ones(img.shape[:2], dtype=np.float32) * mq5_standardized
        ...
        return img, mq8_channel, mq5_channel, temp_min_channel, temp_max_channel, temp_unit_channel, has_coordinates_channel
    ...
    def __getitem__(self, idx):
        ...
        img, mq8_channel, mq5_channel, temp_min_channel, temp_max_channel, temp_unit_channel, has_coordinates_channel = self._add_feature_channels(image, idx)
        ...
        mq8_tensor = torch.tensor(mq8_channel, dtype=torch.float32).unsqueeze(0)
        mq5_tensor = torch.tensor(mq5_channel, dtype=torch.float32).unsqueeze(0)
        ...
        multi_channel_img = torch.cat([img, mq8_tensor, mq5_tensor, temp_min_tensor, temp_max_tensor, temp_unit_tensor, has_coordinates_tensor], dim=0)
...
train_transform = A.Compose([
    ...
    #A.RandomResizedCrop(size=(224,224), scale=(0.5, 1), ratio=(1,1), p=1, interpolation=1),
...
class MultiChannelGasClassifierModule(pl.LightningModule):
    def __init__(self, num_classes=4, learning_rate=0.001):
        ...
        # Replace the first conv layer to accept 9 channels (3 RGB + 6 feature channels)
        ...
        with torch.no_grad():
            ...
            self.backbone.conv1.weight[:, 3:, :, :] = rgb_mean.expand(-1, 6, -1, -1)
        ...
        self.fc1 = nn.Linear(in_features+6,256)
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.act(x)
        x = torch.cat([x, x_[:, 3:, 0, 0].squeeze()], dim=1)
        x = self.fc1(x)
        x = self.act(x)
        x = self.fc2(x)
        x = self.act(x)
