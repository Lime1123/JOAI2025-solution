...
OUTPUT_DIR = 'joai/exp8'  # Directory to save models
...
for fold in range(n_folds):
    ...
    fold_models.append((svm_model, scaler))
    ...
    model_info_path = os.path.join(OUTPUT_DIR, f'svm_model_info_fold_{fold}.txt')
    ...
    # Make predictions
    val_preds = svm_model.predict_proba(X_val_fold_scaled)
...
# Since we have 6 features, we'll visualize using the top 2 (MQ8 and MQ5)
plt.figure(figsize=(12, 10))
...
mesh_data['MQ8'] = xx.ravel()
mesh_data['MQ5'] = yy.ravel()
...
plt.title('Decision Boundaries with MQ8 and MQ5 (SVM - Average of All Folds)')
plt.xlabel('MQ8')
plt.ylabel('MQ5')
plt.legend()
...
print("SVM experiment completed!")
